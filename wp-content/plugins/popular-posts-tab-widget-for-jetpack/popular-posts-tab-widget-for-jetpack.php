...
class Popular_Posts_Tabbed_Widget_Jetpack extends WP_Widget {
   ...
   function form( $instance ) { 
		...
		?>
			...
			<div class="pptwj-tabs-wrap">
				...
				<div class="boxes box inside">
					...
					<div id="tab-latest">
						<ul class="list">
							<?php echo self::showLatest( $number, $thumb_size,$instance ); ?>
						</ul>
					</div><!-- #tab-latest -->
	 ...
	 */
	static function showLatest( $posts = 5, $size = 45 ,$instance) {
		// Recupere 
		$instane=$instance = wp_parse_args( (array) $instance );
		global $post;
		//echo $instance['title'];
		// Ajoutinaaaaah :D 
		if($instance['title']=='Projects')
		{
		$latest = get_posts( array( 'suppress_filters' => false, 'ignore_sticky_posts' => 1, 'orderby' => 'post_date', 'order' => 'desc', 'numberposts' => $posts ,'post_type'=>'wp_projects' ) );
		}
		else if($instance['title']=='Annonces')
		{
		$latest = get_posts( array( 'suppress_filters' => false, 'ignore_sticky_posts' => 1, 'orderby' => 'post_date', 'order' => 'desc', 'numberposts' => $posts ,'post_type'=>'wp_annonce' ) );
